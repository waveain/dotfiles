...
`Ctrl+J` は ASCII 上 LF（0x0a）と同一であるため、環境によって正しく伝わりません。

| 環境 | 下移動 |
|------|--------|
| tmux ペイン移動 | `C-a j`（プレフィックス + j） |
| nvim ウィンドウ移動（tmux 内） | `C-j` |
| nvim ウィンドウ移動（VSCode ターミナル） | `C-w j` |

その他の方向（`C-h` / `C-k` / `C-l`）は tmux・VSCode どちらでもそのまま使えます。
